...
function App() {
  ...
  return <>
    ...
    <main data-live-analysis="true">
      ...
      <section id="analysis" className="analysis-state"><div><small>03 / R-GAP CALCULATION STATUS</small><h2>실측값과 추정값을<br /><em>구분합니다.</em></h2><p>{snapshot?.analysis.message || 'KTO API 연결이 완료되면 실시간 방문자 실측값을 불러옵니다.'}</p><div className="readiness"><span>방문자 수 <b>실시간 연동</b></span><span>체류 강도 <b>{snapshot?.observed_indices.stay_intensity != null ? '실시간 연동' : '적재 대기'}</b></span><span>관광소비 강도 <b>{snapshot?.observed_indices.spend_intensity != null ? '실시간 연동' : '적재 대기'}</b></span><span>관광 다양성 <b>{snapshot?.observed_indices.visitor_diversity != null ? '실시간 연동' : '적재 대기'}</b></span></div></div><article><small>현재 R-GAP</small><strong>산출 준비 중</strong><p>관광수요·다양성 지표까지 연결됐습니다. AGENTS.md 산식에 필요한 아래 항목을 채운 뒤 75분위 프론티어와 실제 TCEI의 차이를 계산합니다.</p><div className="badges">{(snapshot?.analysis.missing_inputs || ['관광지 집중도', '월별 계절성', '숙박공급·접근성', '75분위 프론티어']).map((item) => <span key={item}>{item}</span>)}</div><a href="#simulator">예산 시나리오 열기 ↓</a></article></section>

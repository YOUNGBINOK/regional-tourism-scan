...
function App() {
  ...
  const axes: Axis[] = [
    ...
    { key: 'stayShare', label: '숙박', diff: null, tier: 'pending', note: '숙박 방문자 비율 데이터 연동 예정' },
    { key: 'dispersion', label: '공간확산', diff: null, tier: 'pending', note: '관광지 집중도 데이터 연동 예정' },
    { key: 'season', label: '계절/시간 안정성', diff: null, tier: 'pending', note: '월별 계절변동계수 데이터 연동 예정' },
  ];
  ...
  return <>
    ...
    <main data-live-analysis="true">
      ...
      <section className="meta"><small>DATA INTERPRETATION / REQUIRED META INFO</small><div><b>실측값과 추정값을 구분합니다.</b><p>방문자수는 한국관광공사 통신 기반 지역별 방문자수 GW의 일별 집계입니다(실측). 체류·소비 강도지수는 실측값을 정규화한 파생지표이며, 유사지역 비교·유형 판별은 규칙 기반 모델 추정값입니다. 숙박·공간확산·계절 안정성은 아직 데이터가 연동되지 않아 향후 분석 가능 지표로 표시합니다.</p></div></section>

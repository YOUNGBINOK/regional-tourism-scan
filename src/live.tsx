...
function App() {
  ...
  // 전국에서 좌표를 확인할 수 있는 시 단위 지역만 지도 핀·선택지로 확장한다. 스캔 전에는
  // 기존 4개 관광거점만 보여준다. KTO 방문자 원천은 구가 있는 시의 구를 "OO시 XX구"로
  // 표기하므로 공백이 있으면 구 단위로 보고 제외한다(구가 있는 시 자체의 집계 코드는 유지).
  const isCityLevel = (name: string) => !name.includes(' ');

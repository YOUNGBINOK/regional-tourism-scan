...
function App() {
  ...
  return <>
    <header><div className="logo"><b>R</b>R-GAP<i /></div><nav><a href="#map">실시간 방문자 지도</a><a href="#diagnosis">실측 진단</a><a href="#analysis">R-GAP 산출 상태</a></nav><button type="button">정책 브리프 PDF ↗</button></header>
    <main data-live-analysis="true">
      ...
      <section id="analysis" className="analysis-state"><div><small>03 / R-GAP CALCULATION STATUS</small><h2>실측값과 추정값을<br /><em>구분합니다.</em></h2><p>{snapshot?.analysis.message || 'KTO API 연결이 완료되면 실시간 방문자 실측값을 불러옵니다.'}</p></div><article><small>현재 R-GAP</small><strong>산출 보류</strong><p>아래 필수 입력값이 도착하기 전에는 시연용 점수로 대체하지 않습니다.</p><div className="badges">{(snapshot?.analysis.missing_inputs || ['체류·숙박', '관광소비', '공간분산', '월별 계절성']).map((item) => <span key={item}>{item}</span>)}</div></article></section>
